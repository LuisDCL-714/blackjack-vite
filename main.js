...
const blackjackModule = (() => {
  ...
  const cpuTurn = (minPoints) => {
      for (let i = 1; i < playersPoints.length; i++) {
          if(playersPoints[i] === 21 || (playersPoints[i] >= 19 && Math.random() <= 0.7)){
              continue;
          }
          let cpuPoints = 0;
  ...
  const makePlayingCard = (playingCard, turn) => {
      ...
      imgPlayingCard.src = `public/assets/cartas/${playingCard}.png`;
...
document.getElementById('btn-new-game').addEventListener('click', () => {
  const numParticipants = document.getElementById('num-players').value;
  blackjackModule.startGame(parseInt(numParticipants));
});